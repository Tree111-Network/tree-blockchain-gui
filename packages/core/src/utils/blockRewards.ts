...
export function calculatePoolReward(height: number): BigNumber {
  if (height === 0) {
    return MOJO_PER_CHIA.times(_tree_prefarm).times(POOL_REWARD);
  }
  ...
  return MOJO_PER_CHIA.times(_tree_per_block).times(POOL_REWARD);
}

export function calculateBaseFarmerReward(height: number): BigNumber {
  if (height === 0) {
    return MOJO_PER_CHIA.times(_tree_prefarm).times(FARMER_REWARD);
  }
  // if (height < 3 * BLOCKS_PER_YEAR) {
  ...
  return MOJO_PER_CHIA.times(_tree_per_block).times(FARMER_REWARD);
}
