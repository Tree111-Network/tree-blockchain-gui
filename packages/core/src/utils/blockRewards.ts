...
export function calculatePoolReward(height: number): BigNumber {
  if (height === 0) {
    return MOJO_PER_TREE.times('21000000').times(POOL_REWARD);
  }
  ...
  return MOJO_PER_TREE.times('0.125').times(POOL_REWARD);
}

export function calculateBaseFarmerReward(height: number): BigNumber {
  if (height === 0) {
    return MOJO_PER_TREE.times('21000000').times(FARMER_REWARD);
  }
  if (height < 3 * BLOCKS_PER_YEAR) {
  ...
  return MOJO_PER_TREE.times('0.125').times(FARMER_REWARD);
}
