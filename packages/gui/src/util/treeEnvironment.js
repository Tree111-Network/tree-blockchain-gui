...
const getTreeVersion = () => {
  let version = null;
  const exePath = getExecutablePath('tree');
  // first see if we can get a tree exe in a standard location relative to where we are
  try {
    ...
    // that didn't work, let's try as if we're in the venv or tree is on the path
    try {
      version = child_process
