...
import type { OfferSummaryAssetInfo, OfferSummaryRecord } from '@tree/api';
import { mojoToCAT, mojoToTree, mojoToCATLocaleString, mojoToTreeLocaleString } from '@tree/core';
...
export function formatAmountForWalletType(amount: string | number, walletType: WalletType, locale?: string): string {
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToTreeLocaleString(amount, locale);
  }
...
export function getNFTPriceWithoutRoyalties(
  ...
  for (const assetType of [OfferAsset.TOKEN, OfferAsset.TREE]) {
    const assetId = offerAssetIdForAssetType(assetType, summary);
    if (assetId) {
      const amountInMojos = offerAssetAmountForAssetId(assetId, summary);
      if (amountInMojos) {
        const amountInTokens = assetType === OfferAsset.TREE ? mojoToTree(amountInMojos) : mojoToCAT(amountInMojos);
