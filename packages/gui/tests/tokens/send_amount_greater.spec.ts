...
test('Confirm that User cannot send a TTREE111 amount greater then in Wallet', async () => {
  ...
  await page.locator('text=Fee *TTREE111 >> input[type="text"]').fill('0.000005');
  ...
  await expect(page.locator('text=Pending Balance0 TTREE111')).toBeVisible();
});
