...
test('Confirm that User cannot send a TTREE111 fee greater then in Wallet', async () => {
  ...
  await page.locator('text=Fee *TTREE111 >> input[type="text"]').fill('10');
  ...
  await expect(page.locator('text=Pending Change0 TTREE111')).toBeVisible();
});
